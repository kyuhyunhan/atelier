...
def test_fts_returns_hits(atelier_env):
    ...
    write_page(
        atelier_env["wiki"] / "wiki" / "entities" / "example-person.md",
    ...
    hits = api.search("placeholder", space="wiki")
...
def test_fts_sanitizes_punctuated_query(atelier_env):
    ...
    from runtime.service import api
    write_page(
        atelier_env["wiki"] / "wiki" / "entities" / "session-note.md",
    ...
    hits = api.search("session-end auto-commit: safety-net!", space="wiki")
...
def test_fts_dedups_multi_chunk_pages(atelier_env):
    ...
    write_page(
        atelier_env["wiki"] / "wiki" / "entities" / "widgety.md",
    ...
    api.reindex(space="wiki", full=True)
    hits = api.search("widget", space="wiki")
