"""Lint rules end-to-end on a small synthetic wiki."""
...
def test_L1_catches_broken_raw_link(atelier_env):
    ...
    out = api.lint(space="wiki", rule_ids=["L1"])
...
def test_L3_source_count_drift_and_fix(atelier_env):
    ...
    api.reindex(space="wiki", full=True)
    out = api.lint(space="wiki", rule_ids=["L3"])
    ...
    out2 = api.lint(space="wiki", rule_ids=["L3"], apply_fixes=True)
    assert out2["fixes_applied"] >= 1
    api.reindex(space="wiki", full=True)
    out3 = api.lint(space="wiki", rule_ids=["L3"])
...
def _l8_source(wiki, eid, *, domain, sensitivity):
    write_page(
        wiki / "raw" / domain / f"{eid}.md",
...
def _l8_claim(wiki, eid, *, derived_from, sensitivity):
    write_page(
        wiki / "graph" / "atomic" / f"{eid}.md",
