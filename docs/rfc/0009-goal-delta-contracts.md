...
|---|---|---|
| **G1** L1 lint + PII guard liveness | ~~`lint.L1 = 0`~~ *(superseded 2026-07-29: L1 measures the path-form subset of the accepted-dangling residual — see the worked-arc appendix; the ratchet `dangling_new = 0` / doctor D8 carries this bar now, and engine issue #88 closed the residual as accepted)*; `guard_liveness.pii_active_patterns = 4` (the maintainer-approved static core, pinned by content) **and** `guard_liveness.seeded_probe_blocked = 1` (the §5.3c execution probe) | ✅ **CLOSED 2026-07-29** — engine half via PR #89 (probe metric, hermetic override, hygiene, hard rule #8); the user file authored verbatim, `sha256 = ebcc07dd4c13e33c7bce92b2e3bc3e2e8f20feb626f350efbc6c6368ac3fd69d` (recompute against `~/.atelier/pii_patterns.txt` to audit); live `{pii_active_patterns: 4, seeded_probe_blocked: 1}` and an end-to-end staged-leak block in the engine repo. The remaining artifact was out-of-tree user config, so this closed as verified work, not a repo-delta goal run (the workflow's unit — a reviewable repo diff — has nothing to carry). |
| **G2** promote predicate | `promote_eligible.total ≤ 30`, `.by_domain.knowledge = 0` | ✅ **CLOSED 2026-07-27** (PR #78, the first live goal-workflow run) — `is_promote_eligible` narrowed to the operational lane: 830 → 23, knowledge 807 → 0; live today at 23 ≤ 30. |
| **G3** lens coverage + project axis | `lens_param_present = 6/6` **plus** the §5.5 behavioural gate; `foreign_ratio ≤ 0.15` with `returned ≥ 20` | ✅ **CLOSED 2026-07-29** (PR #95, the full goal-workflow run) — lens 1/6 → **6/6**, every wired surface filtering its results through `lenses.lens_admits_fm`; `recall_v7.project_scope_gate` cut `foreign_ratio` **0.6986 → 0.0** (foreign 51 → 0, own held at 14, returned 26) against the §5.6 fixture, content-pinned; behavioural gate landed as dev-vs-full differing-result tests on four surfaces. Verified twice: the workflow's verifier (lexical-vs-lexical, consistent engines) and an independent re-run — during which the §6 eval-engine guard fired its first live abort on a deliberate engine-mismatched invocation, exactly as designed. The contract took **three critic rounds** to earn acceptance: a pinned-but-absent fixture (run 1), then waiver floors *below* baseline on metrics with no invariant backstop (mrr, dark_count) — an undetectable regression window the final contract closed with full-precision at-baseline floors. **This closes the §7 catalogue: G1–G5 all landed.** |
| **G4** pending review surface | the surface returns **all** pending claims with ages, asserted equal to `pending_age.count`/`.max` | ✅ **CLOSED 2026-07-29** — `review_pending` gains `total`/`age_days`/`max_age_days` (limit pages items only; no silent cap), and both sides now share ONE predicate (`claims_io.is_pending_review`: the operational acceptance queue — before this, the metric counted any-domain pendings and matched the surface only by coincidence). Equality asserted by test and verified live (42/42, 44/44). The queue's judgements stay human. |
| **G5** auto-pass narrowing; wiki-link repair | passed-pool delta (exact, with INV-4 in `supersedes`); ~~dangling-link count → 0~~ *(superseded 2026-07-28: the residual is documented judgment, not backlog — see the worked-arc appendix; the ratchet `dangling_new = 0` / doctor D8 carries the bar)* | ✅ **CLOSED 2026-07-28** (PRs #83/#85/#86 + the arc appendix) — two identity-alias resolver fixes took dangling wikilinks 387 → 128; the arc closed at the ratchet (raw frozen, `dangling_new = 0`), never driven to a fabricating zero. |
